...
public class FitnessDashboardController implements Initializable {
    ...
    private void buildNavMap() {
        ...
        // NOTE: btnCourses is intentionally excluded from navMap —
        // it loads coursfront.fxml dynamically into contentArea (see handleCoursesNav).

        allViews = new ArrayList<>(navMap.values());
        if (viewCourses       != null) allViews.add(viewCourses);
    ...
    public void handleNav(ActionEvent e) {
        ...
        if (src == btnQuiz)    { handleQuizNav();    return; }
        if (src == btnCourses) { handleCoursesNav(); return; }
        if (src == btnEvents)  { showEventsView();   return; }
    ...
    /**
     * Courses button: always reloads coursfront.fxml from DB into contentArea.
     * Behaviour preserved from Document 1.
     */
    private void handleCoursesNav() {
        ...
        try {
            ...
            showAlert("❌ Erreur", "Impossible de charger les cours : " + ex.getMessage());
        }
    ...
    private void setActiveNav(Button active) {
        ...
        if (btnCourses != null) btnCourses.setStyle(btnCourses == active ? activeStyle : inactiveStyle);
    ...
    @FXML public void goToRelax(ActionEvent e) { showView(viewRelax); setActiveNav(btnRelax); }

    public void showViewMedecin() { showView(viewMedecin); setActiveNav(btnRelax); }

    @FXML public void goToMedecin(ActionEvent e) { showView(viewMedecin); setActiveNav(btnRelax); }

    // =========================================================================
    // CHATBOT / CABINET / RESPIRATION / À PROPOS
    // =========================================================================
    public void handleOpenChatbot() {
        if (chatbotArea.getChildren().isEmpty()) {
            try {
                ...
                chatbotArea.getChildren().setAll(errorLabel("chatbot", ex));
            }
        }
        showView(viewChatbot); setActiveNav(btnRelax);
    }
    ...
    public void handleOpenCabinet() {
        if (cabinetArea.getChildren().isEmpty()) {
            try {
                ...
                cabinetArea.getChildren().setAll(errorLabel("cabinet", ex));
            }
        }
        showView(viewCabinet); setActiveNav(btnRelax);
    }
    ...
    public void handleOpenRespiration() {
        try {
            ...
            respirationArea.getChildren().setAll(errorLabel("exercice", ex));
        }
        showView(viewRespiration); setActiveNav(btnRelax);
    }
    ...
    public void handleOpenAPropos() {
        if (aProposArea.getChildren().isEmpty()) {
            try {
                ...
                aProposArea.getChildren().setAll(errorLabel("page À Propos", ex));
            }
        }
        showView(viewAPropos); setActiveNav(btnRelax);
    }

    @FXML public void goToStressOptions(ActionEvent e) { showView(viewMedecin); setActiveNav(btnRelax); }
    ...
    public void handleVip(ActionEvent e) {
        ...
        showView(viewVip); setActiveNav(btnRelax);
    }
    ...
    public void handleOpenSleep() {
        ...
        showView(viewSleep); setActiveNav(btnRelax);
    }

    @FXML public void goToVip(ActionEvent e) { showView(viewVip); setActiveNav(btnRelax); }
    ...
    public void handleEnvoyerReclamation(ActionEvent e) {
        showView(viewReclamation); setActiveNav(btnRelax);
    }
    ...
    // =========================================================================
    // RELAXATION — Médecin / Stress survey
    // =========================================================================
    @FXML
    public void handleConsulterMedecin(ActionEvent e) {
        loadConsultationForm();
        showView(viewMedecin); setActiveNav(btnRelax);
    }
    ...
    public void loadConsultationForm() {
        try {
            ...
            medecinArea.getChildren().setAll(errorLabel("formulaire", ex));
        }
    }
    ...
    public void handleFormSubmitSuccess() {
        ...
        try {
            ...
            medecinArea.getChildren().setAll(errorLabel("options stress", ex));
        }
        showView(viewMedecin); setActiveNav(btnRelax);
    }
    ...
    public void handleCalculerScore(ActionEvent e) {
        try {
            ...
            stressArea.getChildren().setAll(errorLabel("formulaire stress", ex));
        }
        showView(viewStress); setActiveNav(btnRelax);
    }
    ...
    public void handleStressSurveySuccess(int surveyId, int sleep, int study) {
        try {
            ...
            wellBeingResultArea.getChildren().setAll(errorLabel("chatbot stress", ex));
        }
        showView(viewWellBeingResult); setActiveNav(btnRelax);
    }
    ...
    public void handleCatchStress(ActionEvent e) {
        if (catchStressArea.getChildren().isEmpty()) {
            ...
            startTimerBtn.setOnAction(ev -> {
                if (!isRunning[0]) {
                    ...
                    timeline[0] = new Timeline(new KeyFrame(Duration.seconds(1), event -> {
                        ...
                        if (seconds[0] % 10 == 0)
                            motivationLabel.setText(motivationMessages[(seconds[0] / 10) % motivationMessages.length]);
                    }));
            ...
            exercisesList.getChildren().addAll(
                    createExerciseCard("1️⃣ Étirements du Cou", "Durée: 2-3 minutes",
                            new String[]{"• Asseyez-vous confortablement, dos droit",
                                    ...
                                    "• Respirez profondément pendant l'exercice"}, "#e3f2fd", "#1976d2"),
                    createExerciseCard("2️⃣ Respiration Profonde 4-7-8", "Durée: 3-4 minutes",
                            new String[]{"• Asseyez-vous confortablement, fermez les yeux",
                                    "• Inspirez par le nez pendant 4 secondes",
                                    "• Retenez votre souffle pendant 7 secondes",
                                    "• Expirez lentement par la bouche pendant 8 secondes",
                                    "• Répétez ce cycle 4 fois minimum",
                                    "• Sentez votre corps se détendre à chaque expiration"}, "#f3e5f5", "#7b1fa2"),
                    createExerciseCard("3️⃣ Étirements des Épaules", "Durée: 2-3 minutes",
                            new String[]{"• Debout ou assis, dos droit",
                                    ...
                                    "• Croisez les bras et étirez le haut du dos (15 sec)"}, "#e8f5e9", "#388e3c"),
                    createExerciseCard("4️⃣ Étirements des Poignets et Mains", "Durée: 2 minutes",
                            new String[]{"• Tendez le bras droit devant vous, paume vers le haut",
                                    ...
                                    "• Serrez les poings fort puis ouvrez grand les mains (10 fois)"}, "#fff3e0", "#f57c00"),
                    createExerciseCard("5️⃣ Marche Active sur Place", "Durée: 3-5 minutes",
                            new String[]{"• Debout, dos droit, regardez devant vous",
                                    ...
                                    "• Ralentissez progressivement les 30 dernières secondes"}, "#fce4ec", "#c2185b"),
                    createExerciseCard("6️⃣ Étirements du Dos (Chat-Vache)", "Durée: 2-3 minutes",
                            new String[]{"• À quatre pattes, mains sous les épaules, genoux sous les hanches",
                                    ...
                                    "• Terminez en position neutre, respirez profondément"}, "#e0f2f1", "#00796b")
            );
            ...
            catchStressContent.getChildren().addAll(title, subtitle, timerBox, exercisesTitle, exercisesScroll, tipsBox, backBtn);
            catchStressArea.getChildren().setAll(catchStressContent);
        }
        showView(viewCatchStress); setActiveNav(btnRelax);
    }
    ...
    private VBox createExerciseCard(String title, String duration, String[] steps,
        ...
        for (String step : steps) {
            ...
            l.setStyle("-fx-font-size: 13px; -fx-text-fill: #333; -fx-wrap-text: true;");
            l.setMaxWidth(Double.MAX_VALUE);
            stepsBox.getChildren().add(l);
        }
    ...
    public void handleFitness(ActionEvent e) {
        if (fitnessArea.getChildren().isEmpty()) {
            ...
            Label durLbl = new Label("Duree de l'exercice (minutes):");
            durLbl.setStyle("-fx-font-size: 14px; -fx-text-fill: #555;");
            Slider slider = new Slider(5, 60, 15);
            Label durVal = new Label("15 minutes");
            durVal.setStyle("-fx-font-size: 14px; -fx-text-fill: #0277bd;");
            slider.valueProperty().addListener((obs, o, n) -> durVal.setText(n.intValue() + " minutes"));

            HBox basicBtns = new HBox(15);
            basicBtns.setAlignment(Pos.CENTER);
            for (String[] b : new String[][]{
                    {"Echauffement","#4fc3f7"},{"Cardio","#039be5"},
                    {"Force","#0277bd"},{"Etirements","#01579b"}}) {
                Button btn = new Button(b[0]);
                btn.setStyle("-fx-background-color: " + b[1] + "; -fx-text-fill: white; "
                        + "-fx-font-size: 14px; -fx-padding: 15 25; -fx-background-radius: 10;");
                basicBtns.getChildren().add(btn);
            }

            Label advLbl = new Label("Exercices Avances:");
            advLbl.setStyle("-fx-font-size: 16px; -fx-font-weight: bold; -fx-text-fill: #0277bd; -fx-padding: 20 0 10 0;");

            HBox advBtns = new HBox(15);
            advBtns.setAlignment(Pos.CENTER);
            for (String[] b : new String[][]{
                    {"Yoga 3D","#7c4dff"},{"Pilates","#651fff"},
                    {"HIIT","#536dfe"},{"Meditation","#3d5afe"}}) {
                Button btn = new Button(b[0]);
                btn.setStyle("-fx-background-color: " + b[1] + "; -fx-text-fill: white; "
                        + "-fx-font-size: 14px; -fx-padding: 15 25; -fx-background-radius: 10;");
                advBtns.getChildren().add(btn);
            }

            Button backBtn = new Button("← Retour");
            backBtn.setStyle("-fx-background-color: #0277bd; -fx-text-fill: white; -fx-font-size: 14px; "
                    + "-fx-padding: 12 30; -fx-background-radius: 10; -fx-cursor: hand;");
            backBtn.setOnAction(ev -> goToRelax(ev));

            Label infoLabel = new Label("Selectionnez un type d'exercice et ajustez la duree selon vos besoins.");
            infoLabel.setStyle("-fx-font-size: 12px; -fx-text-fill: #888; -fx-padding: 10 0 0 0;");

            content.getChildren().addAll(title, durLbl, slider, durVal, basicBtns, advLbl, advBtns, backBtn, infoLabel);
            fitnessArea.getChildren().setAll(content);
        }
        showView(viewFitness); setActiveNav(btnRelax);
    }
    ...
    public void handleNutrition(ActionEvent e) {
        if (nutritionArea.getChildren().isEmpty()) {
            ...
            Label foodLbl = new Label("Aliments pour booster votre energie:");
            foodLbl.setStyle("-fx-font-size: 14px; -fx-text-fill: #555;");

            FlowPane foodGrid = new FlowPane(15, 15);
            foodGrid.setPrefWrapLength(500);
            for (String[] f : new String[][]{
                    {"Banane","89"},{"Avocat","160"},{"Noix","185"},{"Miel","64"},
                    {"Chocolat Noir","170"},{"The Vert","2"},{"Blueberries","57"},
                    {"Saumon","208"},{"Eggs","78"},{"Legumes Verts","35"}}) {
                VBox fc = new VBox(5);
                fc.setStyle("-fx-background-color: white; -fx-background-radius: 10; -fx-padding: 15; -fx-min-width: 100;");
                Label fn = new Label(f[0]); fn.setStyle("-fx-font-size: 13px; -fx-font-weight: bold;");
                Label fc2 = new Label(f[1] + " cal"); fc2.setStyle("-fx-font-size: 12px; -fx-text-fill: #2e7d32;");
                fc.getChildren().addAll(fn, fc2);
                foodGrid.getChildren().add(fc);
            }
            ...
            Label wLbl = new Label("Poids (g):"); wLbl.setStyle("-fx-font-size: 14px;");
            TextField wField = new TextField("100"); wField.setStyle("-fx-pref-width: 80; -fx-padding: 5;");
            Label cLbl = new Label("Calories pour 100g:"); cLbl.setStyle("-fx-font-size: 14px;");
            TextField cField = new TextField("89"); cField.setStyle("-fx-pref-width: 80; -fx-padding: 5;");
            ...
            Label result = new Label("Resultat: 89 cal");
            result.setStyle("-fx-font-size: 16px; -fx-font-weight: bold; -fx-text-fill: #2e7d32;");
            calcBtn.setOnAction(ev -> {
                try {
                    double w = Double.parseDouble(wField.getText());
                    double c = Double.parseDouble(cField.getText());
                    result.setText("Resultat: " + String.format("%.0f", (w * c) / 100) + " cal");
                } catch (NumberFormatException ex) { result.setText("Erreur: Entrez des nombres"); }
            });
            calcRow.getChildren().addAll(wLbl, wField, cLbl, cField, calcBtn, result);

            Label joyLbl = new Label("Aliments pour ameliorer l'humeur:");
            joyLbl.setStyle("-fx-font-size: 14px; -fx-text-fill: #555; -fx-padding: 20 0 10 0;");
            FlowPane joyGrid = new FlowPane(15, 15);
            joyGrid.setPrefWrapLength(500);
            for (String[] j : new String[][]{{"Chocolat","170","🍫"},{"Fraises","32","🍓"},{"Mangue","60","🥭"},{"The Matcha","2","🍵"}}) {
                HBox jc = new HBox(10);
                jc.setStyle("-fx-background-color: #fff8e1; -fx-background-radius: 10; -fx-padding: 15;");
                Label em = new Label(j[2]); em.setStyle("-fx-font-size: 24px;");
                VBox ji = new VBox(2);
                Label jn = new Label(j[0]); jn.setStyle("-fx-font-size: 13px; -fx-font-weight: bold;");
                Label jc2 = new Label(j[1] + " cal - Bonheur!"); jc2.setStyle("-fx-font-size: 11px; -fx-text-fill: #f57f17;");
                ji.getChildren().addAll(jn, jc2);
                jc.getChildren().addAll(em, ji);
                joyGrid.getChildren().add(jc);
            }

            Button backBtn = new Button("← Retour");
            backBtn.setStyle("-fx-background-color: #2e7d32; -fx-text-fill: white; -fx-font-size: 14px; "
                    + "-fx-padding: 12 30; -fx-background-radius: 10; -fx-cursor: hand;");
            backBtn.setOnAction(ev -> goToRelax(ev));

            content.getChildren().addAll(title, foodLbl, foodGrid, calcTitle, calcRow, joyLbl, joyGrid, backBtn);
            nutritionArea.getChildren().setAll(content);
        }
        showView(viewNutrition); setActiveNav(btnRelax);
    }
    ...
    private VBox buildProduitCard(edu.connexion3a36.entities.Produit p, Map<Integer, String> catMap) {
        ...
        btnAjouter.setOnAction(ev -> {
            ...
            pause.setOnFinished(pe -> { btnAjouter.setText("＋ Ajouter"); btnAjouter.setStyle(styleNormal); });
            pause.play();
        });
    ...
    @FXML public void handleOpenCart(javafx.scene.input.MouseEvent e) {
        showView(viewCart); renderCart(); setActiveNav(btnBoutique);
    }

    @FXML public void handleOpenBoutique(ActionEvent e) {
        showView(viewBoutique); setActiveNav(btnBoutique);
    }

    @FXML public void handleViderCart(ActionEvent e) {
        CartService.getInstance().vider(); updateCartBadge(); renderCart();
    }
    ...
    private HBox buildCartRow(CartItem item) {
        ...
        row.setStyle("-fx-background-color: white; -fx-background-radius: 14; -fx-border-color: #f0f0f0; -fx-border-radius: 14;");
        ...
        suppr.setStyle("-fx-background-color: #fff0f0; -fx-text-fill: #e24b4a; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 4 8;");
        suppr.setOnAction(e -> { CartService.getInstance().supprimerItem(item); updateCartBadge(); renderCart(); });
    ...
    @FXML public void handleBackToCart(ActionEvent e) { showView(viewCart); renderCart(); }
    ...
    public void handleConfirmerPaiement(ActionEvent e) {
        ...
        if (nom.isEmpty() || email.isEmpty() || adresse.isEmpty()) {
            showAlert("Champs manquants", "Veuillez remplir tous les champs."); return;
        }
        if (!email.contains("@")) {
            showAlert("Email invalide", "Veuillez entrer un email valide."); return;
        }
        List<CartItem> items = CartService.getInstance().getItems();
        ...
        try {
            ...
            new Thread(() -> EmailService.envoyerConfirmation(email, nom, items, total)).start();
            CartService.getInstance().vider();
            updateCartBadge();
            showAlert("✅ Commande confirmée !", "Redirection vers Stripe...\nUn email de confirmation sera envoyé à : " + email);
    ...
    private HBox buildCourseRow(Course c) {
        ...
        Label emoji = new Label(c.emoji()); emoji.setStyle("-fx-font-size: 22px;");
        ...
        Label pct = new Label(c.progress() + "%"); pct.setStyle("-fx-font-size: 12px; -fx-font-weight: bold;");
        ProgressBar pb = new ProgressBar(c.progress() / 100.0);
        pb.setPrefWidth(80); pb.setStyle("-fx-accent: #111111;");
        progressBox.getChildren().addAll(pct, pb);

        Label rating = new Label("⭐ " + c.rating()); rating.setStyle("-fx-font-size: 11px; -fx-text-fill: #888;");
    ...
    public void handleFilter(ActionEvent e) {
        ...
        String inactiveStyle = "-fx-background-color: transparent; -fx-text-fill: #888; -fx-cursor: hand; -fx-padding: 4 0;";
    ...
    private void loadChartData(String type) {
        ...
        if ("hours".equals(type)) {
            ...
            double[] vals  = {2, 4, 3, 5};
            for (int i = 0; i < weeks.length; i++) series.getData().add(new XYChart.Data<>(weeks[i], vals[i]));
    ...
    public void handleChartTab(ActionEvent e) {
        ...
        String activeStyle   = "-fx-background-color: transparent; -fx-font-weight: bold;"
                ...
                + " -fx-border-color: transparent transparent #111 transparent; -fx-border-width: 0 0 2 0;";
        String inactiveStyle = "-fx-background-color: transparent; -fx-font-size: 12px; -fx-text-fill: #aaa; -fx-cursor: hand;";
    ...
    @FXML public void handlePeriodChange(ActionEvent e) { loadChartData("hours"); }
    ...
    private HBox buildMessageRow(String[] m) {
        ...
        row.setStyle("-fx-background-color: " + bg + "; -fx-background-radius: 14; -fx-border-color: #f0f0f0; -fx-border-radius: 14;");
    ...
    private void initSettings() {
        ...
        notif.forEach(s -> settingsNotifList.getChildren().add(buildSettingRow(s[0], "true".equals(s[1]))));
        privacy.forEach(s -> settingsPrivacyList.getChildren().add(buildSettingRow(s[0], "true".equals(s[1]))));
    }
    ...
    public void handleChangePassword(ActionEvent e) {
        ...
        VBox content = new VBox(10, new Label("Nouveau mot de passe :"), newMdp, new Label("Confirmer :"), confirmMdp);
    ...
    private VBox buildCourseCard(Course c) {
        ...
        Label author = new Label("par " + c.author()); author.setStyle("-fx-font-size: 11px; -fx-text-fill: #888;");
        ProgressBar pb = new ProgressBar(c.progress() / 100.0);
        pb.setPrefWidth(Double.MAX_VALUE); pb.setStyle("-fx-accent: #111111;");
        Label pct = new Label(c.progress() + "% terminé"); pct.setStyle("-fx-font-size: 11px; -fx-text-fill: #555;");
        ...
        if (emailChange) {
            new Thread(() -> {
                ...
                javafx.application.Platform.runLater(() -> {
                    if (!envoye) { showAlert("❌ Erreur", "Impossible d'envoyer le code. Vérifiez l'email."); return; }
                    try {
                        ...
                        otpStage.show();
                    } catch (IOException ex) { showAlert("❌ Erreur", "Erreur OTP : " + ex.getMessage()); }
                });
            }).start();
        } else {
            try {
            ...
            } catch (SQLException ex) { showAlert("❌ Erreur", "Erreur mise à jour : " + ex.getMessage()); }
        }
    }
    ...
    private void appliquerFiltresEtTri() {
        ...
        else if ("Z-A".equals(sortVal))        cmp = Comparator.comparing((Event ev) -> (ev.getTitre() != null ? ev.getTitre() : "")).reversed();
        else if ("Plus anciens".equals(sortVal)) cmp = Comparator.comparing(Event::getDateCreation, Comparator.nullsLast(Comparator.naturalOrder()));
        ...
        eventsFiltres = tousLesEvents.stream()
                .filter(ev -> {
                    if (!"all".equals(currentTypeFilter) && (ev.getType() == null || !ev.getType().equalsIgnoreCase(currentTypeFilter))) return false;
    ...
    private void refreshEventDisplay() { if (eventsFiltres != null) afficherCartes(eventsFiltres); }
    ...
    private void afficherCartes(List<Event> events) {
        ...
        for (int i = 0; i < events.size(); i++) {
            ...
            ft.setToValue(1); ft.setDelay(Duration.millis(i * 60L));
            TranslateTransition tt = new TranslateTransition(Duration.millis(350), carte);
            tt.setToY(0);     tt.setDelay(Duration.millis(i * 60L));
    ...
    private VBox creerCarteGrid(Event event) {
        ...
        VBox carte = new VBox();
        carte.setPrefWidth(270); carte.setMaxWidth(270);
        ...
        if (event.getDateCreation() != null) {
            try { dateStr = event.getDateCreation().toLocalDate().format(DateTimeFormatter.ofPattern("dd MMM yyyy")); }
        ...
        if (event.getDescription() != null && !event.getDescription().isEmpty()) {
            ...
            Label descL = new Label(desc); descL.setStyle("-fx-font-size: 11px; -fx-text-fill: #5a5a7a;"); descL.setWrapText(true);
        ...
        carte.setOnMouseEntered(e -> { ScaleTransition st = new ScaleTransition(Duration.millis(180), carte); st.setToX(1.025); st.setToY(1.025); st.play(); });
        carte.setOnMouseExited(e  -> { ScaleTransition st = new ScaleTransition(Duration.millis(180), carte); st.setToX(1.0);   st.setToY(1.0);   st.play(); });
        return carte;
    }
    ...
    private void ouvrirDetailEvenement(Event event) {
        ...
        String dateStr = "";
        if (event.getDateCreation() != null) {
            try { dateStr = event.getDateCreation().toLocalDate().format(DateTimeFormatter.ofPattern("EEEE d MMMM yyyy", Locale.FRENCH)); }
            catch (Exception ignored) {}
        }
        ...
        if (event.getDescription() != null && !event.getDescription().isEmpty()) {
            ...
            descTxt.setStyle("-fx-font-size: 13px; -fx-text-fill: #aaaacc;"); descTxt.setWrapText(true);
        ...
        commScroll.setFitToWidth(true); commScroll.setPrefHeight(140);
    ...
    private VBox creerSectionSponsors(int eventId) {
        ...
        try { sponsors = sponsorService.recupererParEvent(eventId); }
    ...
    private void handleRecommandationsIA() {
        try {
            ...
            if (events.isEmpty()) { showAlert("Aucun événement", "Pas d'événements disponibles."); return; }
            new Thread(() -> {
                try {
                    edu.connexion3a36.services.RecommendationService rs = new edu.connexion3a36.services.RecommendationService();
                    String resultat = rs.recommander(events);
                    javafx.application.Platform.runLater(() -> showAlert("🤖 Recommandations IA", resultat));
                } catch (Exception e) {
                    javafx.application.Platform.runLater(() -> showAlert("❌ Erreur", e.getMessage()));
                }
            }).start();
    ...
    private Label defaultEmoji() {
        Label l = new Label("✦"); l.setStyle("-fx-font-size: 42px; -fx-text-fill: #7c6fff;"); return l;
    }

    /** Returns a styled error Label to show inside a StackPane when an FXML load fails. */
    private Label errorLabel(String component, IOException ex) {
        Label l = new Label("❌ Impossible de charger " + component + " : " + ex.getMessage());
        l.setStyle("-fx-text-fill: #e24b4a; -fx-font-size: 13px; -fx-padding: 24;");
        return l;
    }
    ...
    private void showAlert(String message) {
        Alert a = new Alert(Alert.AlertType.INFORMATION);
        a.setHeaderText(null); a.setContentText(message);
        a.showAndWait();
    }
}